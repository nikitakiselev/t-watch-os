...
struct Program {
    ...
    void (*onResume)();
    // Действие короткого клика ТОЛЬКО на корне (главный экран), где «назад» уже
    // некуда — например, уход в сон. На остальных экранах короткий клик ВСЕГДА
    // делает «назад» (kernelBack) на уровне ядра и сюда не приходит. nullptr —
    // на корне клик ничего не делает.
    void (*onButton)();
};
...
void           kernelHome();                  // закрыть всё, вернуться на главный экран (корень)
bool           kernelAtRoot();                // true, если текущая программа — корень (главный экран)
